...
def dashboard():
    ...
    # Modify the query to fetch B_GROUP and TOTAL_PACKETS
    cur.execute("SELECT B_GROUP, TOTAL_PACKETS FROM BLOODBANK")
...
def accept():
    ...
    allnotifications = cur.fetchall()

    for row in allnotifications:
        group = row['NB_GROUP']
        packet = row['N_PACKETS']

        # Check if enough packets are available in the blood bank
        cur.execute("SELECT TOTAL_PACKETS FROM BLOODBANK WHERE B_GROUP = %s", (group,))
        total_packets = cur.fetchone()['TOTAL_PACKETS']

        if total_packets >= packet:
            # Update BLOODBANK
            cur.execute("UPDATE BLOODBANK SET TOTAL_PACKETS = TOTAL_PACKETS - %s WHERE B_GROUP = %s", (packet, group))
            
            # Remove the entry from the contact table
            cur.execute("DELETE FROM CONTACT WHERE B_GROUP = %s AND C_PACKETS = %s", (group, packet))

            # Remove the entry from the notification table
            cur.execute("DELETE FROM NOTIFICATIONS WHERE NB_GROUP = %s AND N_PACKETS = %s", (group, packet))
            flash('Requests Accepted', 'success')
        else:
            flash(f'Not enough blood packets available for blood group {group}', 'danger')
...
@app.route('/notifications/decline', methods=['POST'])
@is_logged_in
def decline():
    cur = mysql.connection.cursor()

    # Get the NB_GROUP and N_PACKETS values from the form data
    group = request.form.get('NB_GROUP')
    packets = request.form.get('N_PACKETS')

    # Remove the entry from the contact table
    cur.execute("DELETE FROM CONTACT WHERE B_GROUP = %s AND C_PACKETS = %s", (group, packets))

    # Remove the entry from the notification table
    cur.execute("DELETE FROM NOTIFICATIONS WHERE NB_GROUP = %s AND N_PACKETS = %s", (group, packets))
